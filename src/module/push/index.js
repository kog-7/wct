...
if(utils.includeKeys(arg,['wctfile','url','object'])===false){
  inquirer.prompt([{name:'confirm',choices:['yes','no'],type:'input',message:`you do not set arg -wctfile or -url or -object,are you sure to push ${utils.cwd}?yes/no`}]).then((ans)=>{
    if(ans.confirm==='yes'){
      loop
...
let wctfileObject={};
let wctValue=arg.wctfile[0];
try{
  wctfileObject=require(nodepath.join(utils.cwd,'wctfile.js')).push;
}
catch(e){
  reject(e);
  return;
  ...
  loop
  .get(promise_infoStore,value)//查询value库的相关信息
  .get(createResource,wctValue)//对当前文件进行查询
  .run();
}
...
.run();
  ...
  try{
    ...
    utils.noLog(`${object[0]} object format error,don't use ",use ' in object`);
    return;
  }

  loop
  .get(promise_infoStore,value)//查询value库的相关信息
  .get(createResource,context.obj)//对当前文件进行查询
  .run();
}
